...
Implementation of the FIR truncated SLS controller using cvxpy.
...
def roll_forward(A, B, K, x0, psi0, sigma_w, horizon, rng=None):
    """Apply an LTI controller

    Roll the true system (A, B) forward with the SS realization of the LTI
    controller given. The horizon length of the trajectory is horizon, and
    the stddev of the Gaussian process noise is sigma_w.
...
def sls_synth(Q, R, Ahat, Bhat, eps_A, eps_B, T, gamma, alpha, logger=None):
    ...
    prob.solve(solver=cvx.SCS, verbose=True, max_iters=5000, eps=1e-3)
...
def sls_common_lyapunov(A, B, Q, R, eps_A, eps_B, tau, logger=None):
    # Taken from:
    # https://github.com/stephentu/lstd-lqr/blob/master/code/policy_iteration.ipynb
    ...
    if prob.status == cvx.OPTIMAL:
        ...
        # TODO: add these assertions back
        #_assert_symmetric_matrix(P_value)
        #assert lambda_min(P_value) >= -1e-10

        # NOTE: the K returned here is meant to be used
        # as A + BK **NOT** A - BK as in the original matlab code
...
#        # TODO(stephentu):
#        # actually search for gamma and not just hardcode a value
#        # we can keep alpha hardcoded
#
...
#
#        # match nominal for debugging purposes
#
#        ## By default, epoch i should have epoch_multiplier * 2^i iterations
#        #if (self._iteration_within_epoch_idx >=
#        #        self._epoch_multiplier * math.pow(2, self._epoch_idx)):
#        #    return True
#        #else:
#        #    return False
#
...
#    """Adaptive control based on common Lyapunov SLS
#
#    """
#
#    def __init__(self, Q, R, A_star, B_star, sigma_w, rls_lam,
...
#        logger = self._get_logger()
#
#        # if SLS is not feasible, we fallback to prev SLS controller if exists, otherwise
#        # we raise an SLSInfeasibleException exception
#
#        Anom, Bnom, _ = utils.solve_least_squares(states, inputs, transitions, reg=self._reg)
#        eps_A = np.linalg.norm(Anom - self._A_star, ord=2)
#        eps_B = np.linalg.norm(Bnom - self._B_star, ord=2)
...
#        #    logger = self._get_logger()
#        #    logger.debug("terminating epoch... exploration noise will now have stddev {}".format(
#        #        self._sigma_explore * 1/(math.pow(2, (self._epoch_idx + 1)/6))))
#        #    return True
#        #else:
#        #    return False
#
