...
from zo_lq_model_modified import ZO_LQ_model
import sys
...
def Simulation(model_params, algo_params):
    ...
    model.sample_size_finer_list = [np.nan for _ in range(memory_length)]
    model.sample_size_list = [np.nan for _ in range(memory_length)]
    Tin = algo_params["Tin"]
    M1 = algo_params["M1"]
    M2 = algo_params["M2"]
    epsilon1 = algo_params["epsilon1"]
    epsilon2 = algo_params["epsilon2"]
    r1 = algo_params["r1"]
    r2 = algo_params["r2"]
    ...
    signature  = str(memory_length) + "_" + str(Tin) + "_" + str(tau1) + "_" + str(tau2) + "_" + str(M1) + "_" + str(M2) \
                    + "_" + str(epsilon1) + "_" + str(epsilon2) + "_" + str(r1) + "_" + str(r2)
    ...
    model.sample_size_finer_list[0] = 0
    model.sample_size_list[0] = 0
    # iteration flag
    model.iter_num = 0
    ...
    while model.iter_num < memory_length-1:
        ...
        if not exact_inner_loop:
            ...
            inner_iter_flag = 0
            while inner_iter_flag < model.Tin and model.iter_num < memory_length-2:
                ...
                model.iter_num += 1
                inner_iter_flag += 1
                print("iter num: ", model.iter_num)
                temp_cost = model.compute_cost(K, L)
                cost_diff = temp_optimal_cost - temp_cost
                cost_list[model.iter_num] = temp_cost
                print("inner loop iteration:", model.iter_num, ": ", temp_cost)
                print("cost diff: ", cost_diff)
                lambda_list[model.iter_num] = model.compute_lambda(K, L)
                model.sample_size_finer_list[model.iter_num] = model.sample_size_finer_list[model.iter_num-1] + model.M1
        ...
        print("iter num: ", model.iter_num)
        print("outer iter num: ", outer_loop_iter_num)
        ''' 
        record convergence measure
        '''
        cost_list[model.iter_num] = model.compute_cost(K, L)
        cost_diff_list[outer_loop_iter_num] = cost_list[model.iter_num] - model.nash_cost
        lambda_list[model.iter_num] = model.compute_lambda(K, L)
        avg_ng_list[outer_loop_iter_num] = model.compute_avg_ng(K, outer_loop_iter_num+1, avg_ng_list[outer_loop_iter_num-1])
        model.sample_size_finer_list[model.iter_num] = model.sample_size_finer_list[model.iter_num-1] + model.M2
        model.sample_size_list[outer_loop_iter_num] = model.sample_size_finer_list[model.iter_num]
        print("outer loop cost: ", model.iter_num, ": ", cost_list[model.iter_num])
        print("outer loop lambda: ", model.iter_num, ": ", lambda_list[model.iter_num])
    ...
    sample_size_list_filename = dir + "/sample_size_list/" + signature + "_" + str(ind) + ".pk"
    sample_size_finer_list_filename = dir + "/sample_size_finer_list/" + signature + "_" + str(ind) + ".pk"
    ...
    if not os.path.isdir(dir + "/sample_size_list/"):
        os.mkdir(dir + "/sample_size_list/")
    if not os.path.isdir(dir + "/sample_size_finer_list/"):
        os.mkdir(dir + "/sample_size_finer_list/")
    ...
    sample_size_file = open(sample_size_list_filename, 'wb+')
    sample_size_finer_file = open(sample_size_finer_list_filename, "wb+")
    ...
    pickle.dump(model.sample_size_list, sample_size_file)
    pickle.dump(model.sample_size_finer_list, sample_size_finer_file)
...
temp_algo_params = {
    ...
    "benchmark_algo": 1,
    # Tin is the uniform inner-loop iteration number, in practice, we cannot
    # utilize epsilon1 to determine 
    # "Tin": int(sys.argv[16])
    "Tin": 10,
    # "Tin": 1,
    "group_num": 5 * 10**4
...
Tin = temp_algo_params["Tin"]
r1 = temp_algo_params["r1"]
r2 = temp_algo_params["r2"]
...
if compare_plot:
    ...
    # plt.ylabel("cost difference", fontsize=18)

    if not os.path.isdir(dir + signature):
        os.mkdir(dir + signature)
    plt.legend()
    plt.savefig(dir + signature + "/compare_cost_diff.pdf", bbox_inches='tight')
    
else: 
    # print("debug: ", temp_cost_diff_list)
    plt.plot(temp_cost_diff_list, label='Our algo')
    plt.yscale('log')
    plt.xlabel('Outer-loop iterations $t$', fontsize=18)
    # plt.ylabel(r'$\mathcal{G}(\mathbf{K}_t,\mathbf{L}(\mathbf{K}_t))-\mathcal{G}(\mathbf{K}^*,\mathbf{L}^*)$', fontsize=18)
...
if compare_plot:
    ...
    # plt.xlabel("total iteration", fontsize=18)
    ...
    if not os.path.isdir(dir + signature):
        os.mkdir(dir + signature)
    plt.savefig(dir + signature + "/compare_cost.pdf", bbox_inches='tight')
else:
...
if compare_plot:
    ...
    # plt.xlabel("total iteration", fontsize=18)
    # plt.ylabel("lambda difference", fontsize=18)

    plt.legend()
    if not os.path.isdir(dir + signature):
        os.mkdir(dir + signature)
    plt.savefig(dir + signature + "/compare_lambda.pdf", bbox_inches='tight')
...
if compare_plot:
    ...
    # plt.xlabel("outer-loop iteration", fontsize=18)
    plt.ylabel('Average natural gradient norm', fontsize=18)
    
    plt.savefig(dir + signature + "/compare_avg_ng_norm.pdf", bbox_inches='tight')
else: 
